...
def main():
    ...
    Ex = np.zeros((nx, ny+1, nz+1), dtype='float32')
    Ey = np.zeros((nx+1, ny, nz+1), dtype='float32')
    Ez = np.zeros((nx+1, ny+1, nz), dtype='float32')
    ...
    for iteration in range(MAXIMUM_ITERATION):
        if iteration%PLOT_MODULUS == 0:
            ...
            plt.savefig("./img/Ez_{}.png".format(iteration))
        ...
        # Update Hx
        Hx[0:nx-1,0:ny,0:nz] = Hx[0:nx-1,0:ny,0:nz] \
                                + dtmudz*(Ey[1:nx, 0:ny,   1:nz+1]-Ey[1:nx, 0:ny, 0:nz]) \
                                - dtmudy*(Ez[1:nx, 1:ny+1, 0:nz]  -Ez[1:nx, 0:ny, 0:nz])

        # Update Hy
        Hy[0:nx,0:ny-1,0:nz] = Hy[0:nx,0:ny-1,0:nz] \
                                + dtmudx*(Ez[1:nx+1, 1:ny, 0:nz]  -Ez[0:nx, 1:ny, 0:nz]) \
                                - dtmudz*(Ex[0:nx,   1:ny, 1:nz+1]-Ex[0:nx, 1:ny, 0:nz])

        # Update Hz
        Hz[0:nx,0:ny,0:nz-1] = Hz[0:nx,0:ny,0:nz-1] \
                              + dtmudy*(Ex[0:nx,   1:ny+1, 1:nz]-Ex[0:nx, 0:ny, 1:nz]) \
                              - dtmudx*(Ey[1:nx+1, 0:ny,   1:nz]-Ey[0:nx, 0:ny, 1:nz])
        ...
        # Update Ex
        Ex[0:nx,1:ny,1:nz] = Ex[0:nx,1:ny,1:nz] \
                                + dtepsdy*(Hz[0:nx, 1:ny,   0:nz-1] - Hz[0:nx, 0:ny-1, 0:nz-1]) \
                                - dtepsdz*(Hy[0:nx, 0:ny-1, 1:nz  ] - Hy[0:nx, 0:ny-1, 0:nz-1])

        # Update Ey
        Ey[1:nx,0:ny,1:nz] = Ey[1:nx,0:ny,1:nz] \
                                + dtepsdz*( Hx[0:nx-1, 0:ny, 1:nz  ] - Hx[0:nx-1,0:ny,0:nz-1] ) \
                                - dtepsdx*( Hz[1:nx,   0:ny, 0:nz-1] - Hz[0:nx-1,0:ny,0:nz-1] )

        # Update Ez
        Ez[1:nx,1:ny,0:nz] = Ez[1:nx,1:ny,0:nz] \
                            + dtepsdx*( Hy[1:nx,   0:ny-1, 0:nz] - Hy[0:nx-1,0:ny-1,0:nz] ) \
                            - dtepsdy*( Hx[0:nx-1, 1:ny,   0:nz] - Hx[0:nx-1,0:ny-1,0:nz] )
