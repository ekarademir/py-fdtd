...
# @vectorize(["float32(float32,float32,float32,float32,float32,float32,float32)"], target='cpu')
@cuda.jit(argtypes=[f4[:,:,:], f4, f4[:,:,:], f4[:,:,:], f4, f4[:,:,:], f4[:,:,:]])
def calc_fields(Hi, c1, Ej1, Ej2, c2, Ek1,Ek2):
    i,j,k = cuda.grid(3)
    if i<Hi.shape[0] and j<Hi.shape[1] and k<Hi.shape[2]:
        Hi[i,j,k] =  Hi[i,j,k] + c1*(Ej1[i,j,k]-Ej2[i,j,k]) - c2*(Ek1[i,j,k]-Ek2[i,j,k])
...
def main():
    ...
    Ex = np.ascontiguousarray(np.zeros((nx,   nypp, nzpp), dtype='float32'))
    # np.ascontiguousarray(Ex, dtype='float32')
    Ey = np.ascontiguousarray(np.zeros((nxpp, ny,   nzpp), dtype='float32'))
    # np.ascontiguousarray(Ey, dtype='float32')
    Ez = np.ascontiguousarray(np.zeros((nxpp, nypp, nz), dtype='float32'))
    ...
    Hx = np.ascontiguousarray(np.zeros((nxmm, ny,   nz), dtype='float32'))
    # np.ascontiguousarray(Hx, dtype='float32')
    Hy = np.ascontiguousarray(np.zeros((nx,   nymm, nz), dtype='float32'))
    # np.ascontiguousarray(Hy, dtype='float32')
    Hz = np.ascontiguousarray(np.zeros((nx,   ny,   nzmm), dtype='float32'))
    ...
    for iteration in range(MAXIMUM_ITERATION):
        if iteration%PLOT_MODULUS == 0:
            ...
            plt.savefig("./img/cuda1_Ez_{}.png".format(iteration))
        ...
        griddim = np.shape(Hx[0:nxmm,0:ny,0:nz])
        cuda_f = calc_fields.configure(griddim, blockdim)
        cuda_f( np.ascontiguousarray(Hx[0:nxmm,0:ny,0:nz]), \
                                    dtmudz, np.ascontiguousarray(Ey[1:nx, 0:ny,   1:nzpp]), np.ascontiguousarray(Ey[1:nx, 0:ny, 0:nz]),\
                                    dtmudy, np.ascontiguousarray(Ez[1:nx, 1:nypp, 0:nz]),   np.ascontiguousarray(Ez[1:nx, 0:ny, 0:nz]))
        ...
        griddim = np.shape(Hy[0:nx,0:nymm,0:nz])
        cuda_f = calc_fields.configure(griddim, blockdim)
        cuda_f( np.ascontiguousarray(Hy[0:nx,0:nymm,0:nz]),
                                    dtmudx, np.ascontiguousarray(Ez[1:nxpp, 1:ny, 0:nz]),  np.ascontiguousarray(Ez[0:nx, 1:ny, 0:nz]),
                                    dtmudz, np.ascontiguousarray(Ex[0:nx,   1:ny, 1:nzpp]),np.ascontiguousarray(Ex[0:nx, 1:ny, 0:nz]))
        ...
        griddim = np.shape(Hz[0:nx,0:ny,0:nzmm])
        cuda_f = calc_fields.configure(griddim, blockdim)
        cuda_f(np.ascontiguousarray(Hz[0:nx,0:ny,0:nzmm]),
                                    dtmudy, np.ascontiguousarray(Ex[0:nx,   1:nypp, 1:nz]), np.ascontiguousarray(Ex[0:nx, 0:ny, 1:nz]),
                                    dtmudx, np.ascontiguousarray(Ey[1:nxpp, 0:ny,   1:nz]), np.ascontiguousarray(Ey[0:nx, 0:ny, 1:nz]))
        ...
        griddim = np.shape(Ex[0:nx,1:ny,1:nz])
        cuda_f = calc_fields.configure(griddim, blockdim)
        cuda_f(np.ascontiguousarray(Ex[0:nx,1:ny,1:nz]),
                                    dtepsdy, np.ascontiguousarray(Hz[0:nx, 1:ny,   0:nzmm]), np.ascontiguousarray(Hz[0:nx, 0:nymm, 0:nzmm]),
                                    dtepsdz, np.ascontiguousarray(Hy[0:nx, 0:nymm, 1:nz  ]), np.ascontiguousarray(Hy[0:nx, 0:nymm, 0:nzmm]))
        ...
        griddim = np.shape(Ey[1:nx,0:ny,1:nz])
        cuda_f = calc_fields.configure(griddim, blockdim)
        cuda_f(np.ascontiguousarray(Ey[1:nx,0:ny,1:nz]),
                                    dtepsdz, np.ascontiguousarray(Hx[0:nxmm, 0:ny, 1:nz]),   np.ascontiguousarray(Hx[0:nxmm,0:ny,0:nz-1]),
                                    dtepsdx, np.ascontiguousarray(Hz[1:nx,   0:ny, 0:nzmm]), np.ascontiguousarray(Hz[0:nxmm,0:ny,0:nz-1]))
        ...
        griddim = np.shape(Ez[1:nx,1:ny,0:nz])
        cuda_f = calc_fields.configure(griddim, blockdim)
        cuda_f(np.ascontiguousarray(Ez[1:nx,1:ny,0:nz]),
                                    dtepsdx, np.ascontiguousarray(Hy[1:nx,   0:nymm, 0:nz]), np.ascontiguousarray(Hy[0:nxmm,0:nymm,0:nz]),
                                    dtepsdy, np.ascontiguousarray(Hx[0:nxmm, 1:ny,   0:nz]), np.ascontiguousarray(Hx[0:nxmm,0:nymm,0:nz]))
